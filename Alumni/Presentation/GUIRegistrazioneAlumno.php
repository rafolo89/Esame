				...
				if((strlen($a) < 4) || (strlen($a) >11 )) {
            	 ...
            	 if( (strlen($i)> 9) || (strlen($i)<4) || ($h != $i) ) {
            	     ...
            	     if($numero==0){
                      ...
                      $queryID="SELECT ID FROM alumno WHERE NOME='$a' && COGNOME='$b' && EMAIL='$g' && ANNO_DI_LAUREA='$f' && LAVORO='$e'";
                      $qID=mysql_query($queryID);
                      $ID=mysql_fetch_array($qID, MYSQL_NUM);
                  if(!(mail($g, "Registrazione avvenuta con successo", "Ti ringraziamo per esserti registrato ad AlumniUnisa. Le tue credenziali per l'accesso sono:  ID = ".$ID[0]." & password :".$h." "))){
